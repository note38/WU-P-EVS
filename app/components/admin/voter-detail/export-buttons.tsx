...
export function ExportButtons({
  ...
  const handleExportUncastPDF = async () => {
    const uncastVoters = voters.filter((v: any) => v.status === "UNCAST");
    
    if (uncastVoters.length === 0) {
      toast({
        title: "No Data",
        description: "No uncast voters to export.",
        variant: "destructive",
      });
      return;
    }
    ...
    setIsExporting(true);
    try {
      const exportData = prepareVoterDataForExport(uncastVoters);
      const result = await exportToPDF(exportData, `${title} - Uncast`);
    ...
    <div className="flex flex-wrap gap-2">
      ...
      <DropdownMenu>
        ...
        <DropdownMenuContent align="end">
          ...
          <DropdownMenuItem onClick={handleExportUncastPDF} disabled={isExporting}>
            <FileTextIcon className="mr-2 h-4 w-4" />
            Export Uncast as PDF
          </DropdownMenuItem>
